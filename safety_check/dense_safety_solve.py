...
def dense_safety_solve(nfeatures,nfilters,filters,bias,input,activations,pcl,pgl,span,numSpan,pk):  
    ...
    s.reset()
    for l in pcl.keys():
#        print("l in prevspan.keys()"+str(l))
        variable[1,0,l+1] = Real('1_x_%s' % (l+1))
#        print("variable[1,0,%d] = Real('1_x_' )" % (l+1))
        d += 1

                        
    for k in span.keys():
 #       print("k in span.keys()"+str(k))
        variable[1,1,k+1] = Real('1_y_%s' % (k+1))
 #       print("variable[1,1,k+1] = Real('1_y_%s' % (k+1))")
        d += 1
