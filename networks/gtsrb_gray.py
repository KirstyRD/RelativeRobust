...
def read_dataset():
    ...
    try:
        with  h5py.File('networks/X_gray.h5') as hf:
            X, Y = hf['imgs'][:], hf['labels'][:]
        X = np.array(map(lambda x: transform.resize(x, (1, IMG_SIZE,IMG_SIZE)), X))
        Y = np.array(map(toVector, Y))
        print("Loaded images from X_gray.h5")
        print(str(X.shape))
        print(str(Y.shape))

    except (IOError,OSError, KeyError, IndexError):
        print("Error in reading X_gray.h5. Processing all images...")
        root_dir = 'networks/gtsrb/Final_Training/Images/'
        imgs = []
        labels = []
        ...
        with h5py.File('networks/X_gray.h5','w') as hf:
            hf.create_dataset('imgs', data=X)
...
def read_test_dataset():
    ...
    try:
        with  h5py.File('networks/X_test_gray.h5') as hf:
            X, Y = hf['imgs'][:], hf['labels'][:]
        X = np.array(map(lambda x: transform.resize(x, (1, IMG_SIZE,IMG_SIZE)), X))
        Y = np.array(map(toVector, Y))
        print("Loaded images from X_test_gray.h5")
        print(str(X.shape))
        print(str(Y.shape))

    except (IOError,OSError, KeyError, IndexError):
        print("Error in reading X_test_gray.h5. Processing all images...")
        root_dir = 'networks/gtsrb/Final_Test/Images/'
        imgs = []
        labels = []
        ...
        with h5py.File('networks/X_test_gray.h5','w') as hf:
            hf.create_dataset('imgs', data=X)
            hf.create_dataset('labels', data=Y+1)
